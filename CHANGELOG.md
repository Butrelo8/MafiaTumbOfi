...
- **Admin CRM wiring (end-to-end):** `**bookings.internal_notes`** + `**bookings.deleted_at**` in `**src/db/schema.ts**` (aligned with `**drizzle/0011_**` / `**0012_**`). API: `**DELETE /api/admin/bookings/:id**` soft-delete; `**GET /api/admin/bookings**` and `**GET /api/admin/export/bookings**` exclude `**deleted_at**`; `**PATCH /api/admin/bookings/:id**` accepts `**internalNotes**` (max 10k) alone or with `**pipelineStatus**`; resend/PATCH require non-deleted row. `**processDripEmails**` (`**src/lib/dripProcessor.ts**`) skips soft-deleted rows. `**web/src/pages/admin.astro**` — search + pipeline pills (page slice), stale **Nuevo** badge, `**tel:`** quick link, CSV export of visible rows, side drawer (detail + save notes via `**/admin/save-internal-notes**`), soft-delete via `**/admin/delete-booking**`, sortable **Fecha evento**. `**web/src/lib/adminBookingDrawer.ts`** — **Notas internas** row. Tests: `**admin-bookings-pipeline.test.ts`**, `**dripProcessor.test.ts**`, `**00-auth-protected.test.ts**` (mock `**db**` chain with `**.where().orderBy().limit()**`).
- **Turso (libsql) for production DB:** `**@libsql/client`**, `**drizzle-orm/libsql**` in `**src/db/index.ts**`; `**src/db/detect.ts**` + `**detect.test.ts**` (remote vs embedded replica vs `**bun:sqlite**`). `**scripts/run-migration.ts**` / `**scripts/check-db.ts**` async + libsql. `**src/lib/findMissingTables.ts**` + `**findMissingTables.test.ts**` (replaces `**findMissingSqliteTables**`). `**render.yaml**` — Turso env vars, **no** persistent disk. `**DEPLOY.md`**, `**.env.example**`, `**drizzle.config.ts**` (Turso driver when env set). `**DECISIONS.md**` — why Turso vs disk/Postgres.
...
- **Default deploy:** API database target is **Turso**; file `**DB_PATH`** remains for local/tests when Turso env is unset.
...
- **Plain-text booking + drip emails:** Replaced `**.filter(Boolean)`** with `**.filter((line): line is string => line !== null)**` on line arrays in `**src/routes/booking.ts**` (band + customer confirmation) and `**buildDripEmail3**` in `**src/lib/dripEmails.ts**` so intentional `**''**` paragraph breaks are kept while conditional `**null**` rows are removed. Tests: `**dripEmails.test.ts**`, `**booking.test.ts**`.
- **Admin booking detail drawer:** Raised `**z-index`** above the marketing `**site-header**` (social + hamburger were painting on top of the panel); slightly wider panel, scrollable detail fields with notes/actions pinned in the column.
- **Migrations on Turso (libsql remote):** `**drizzle/0003_confirmation_delivery_fields.sql`** and `**drizzle/0010_admin_explicit_clerk_ids.sql**` — insert `**--> statement-breakpoint**` between adjacent statements so `**bun run migrate**` sends one statement per `**execute()**` (avoids `**SQL_MANY_STATEMENTS**`; `**bun:sqlite**` `**run**` accepted multi-statement chunks).
...
- **check-db:** `scripts/check-db.ts` requires both `users` and `bookings` (parameterized `sqlite_master` checks via `findMissingTables` in `src/lib/findMissingTables.ts`); logs `[check-db] OK` on success. Tests: `src/lib/findMissingTables.test.ts`.
...
- booking.astro: default PUBLIC_API_URL to [http://localhost:3001](http://localhost:3001) (was 3000)
